...
const patchProject = (req, res) => {
    if (req.params.id * 1 > projectlist.length) {
        ...
        const id = req.params.id * 1;
        const dataToUpdate = projectlist.find(el => el.id == id)
        dataToUpdate.projectName = req.body['projectName']
        projectlist.splice(id-1, 1, dataToUpdate)
        console.log("final",projectlist)
        fs.writeFile(`${__dirname}/dev-data/jsonData/db.json`, JSON.stringify(projectlist), err => {
            console.log(err);
        })
        res.status(201)
